<?php
	...
	function get_borrower_info() {
		...
		$this->db->join("loan l2","borrower.borrower_id=l2.borrower_id","LEFT OUTER JOIN");
	  	...
	  	//$query = $this->db->get('borrower');*/
		$sql="select borrower.firstname,borrower.lastname,borrower.email,borrower.city,borrower.address,borrower.borrower_id,borrower.dob,borrower.mobile,loan.status,borrower.docs
	...
	//Get the installment row count
	function get_installment_count($id) {
	  $query = $this->db->get_where('installment',array('loan_id'=>$id));
	  return $query->num_rows();	
	}
	
	
	//Model to get loan details
	...
	function get_last_installment($id) {
