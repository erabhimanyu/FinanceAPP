<?php
	...
	function get_installment_byloan_u() {
	...
	/* Updated loan and borrower */
	function get_installment_byloan() {
		$this->db->select('loan.*, borrower.*');
		$this->db->from('loan');
		$this->db->join('borrower','loan.borrower_id=borrower.borrower_id');
		//$this->db->join('installment','loan.loan_id = installment.loan_id', 'left outer');
		//$this->db->order_by('installment.insta_id','desc');
		//$this->db->limit(10);
	  	$query = $this->db->get();
	 	return $query->result_array();
