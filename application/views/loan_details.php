			...
			<table id='tab1' class='table table-bordered table-striped'>
				...
				<tbody>
					...
					<tr>
						...
						<td name= "id" style="display:none;"><?php echo $row['loan_id']; ?></td>
						<td>
							<?php 
								if($row['status']=="1") {
...
<script type="text/javascript">
	...
	$(document).ready(function(){
	  $('#loaninsta').click(function () {
		var selectedItem = $('#tab1').find(':checkbox:checked').parents('tr');
		var td = $(selectedItem).children('td');  
		var loan_id = td[8].innerText;
		var slvals = []
		...
		if(id.length>1 || id.length<1) {
			...
			window.location=base_url+"borrower/payinstallment/"+id+"/"+loan_id;	
